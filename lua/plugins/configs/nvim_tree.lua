...
if not status1 then vim.notify("Failed to load: " .. plugin1_name, "error", { title = plugin1_name }) return end

local plugin2_name = "nvim-tree.config"
local status2, plugin2 = pcall(require, plugin2_name)
if not status2 then vim.notify("Failed to load: " .. plugin2_name, "error", { title = plugin2_name }) return end
